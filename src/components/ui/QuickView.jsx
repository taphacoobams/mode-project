    ...
    <Modal isOpen={isOpen} onClose={onClose} className="p-0 overflow-hidden">
      <div className="flex flex-col md:flex-row w-full">
        ...
        <div className="hidden md:flex flex-col gap-2 p-2 bg-gray-100 w-16">
          <div className="border border-primary p-1 cursor-pointer">
            <img 
              src={product.image} 
          ...
          {product.additionalImages?.slice(0, 3).map((img, index) => (
            <div key={index} className="border border-gray-200 p-1 cursor-pointer hover:border-primary transition-colors">
              <img 
                src={img} 
        ...
        <div className="p-6 md:w-1/3 flex flex-col justify-center">
          <h2 className="text-2xl font-serif font-bold text-secondary mb-2">{product.name}</h2>
          ...
          <Link 
            ...
            className="inline-block bg-primary hover:bg-primary/90 text-white py-3 px-6 text-sm uppercase tracking-wider font-medium w-full text-center"
          >
            Voir les détails
          </Link>
        </div>
